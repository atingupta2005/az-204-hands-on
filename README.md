# AZ-204T00-A: Developing Solutions for Microsoft Azure

## Module 01: Creating Azure App Service Web Apps
- [Section-4-Develop-Azure-compute-solutions-Azure-Web-Apps-and-Azure-Functions](Section-4-Develop-Azure-compute-solutions-Azure-Web-Apps-and-Azure-Functions)

## Module 02: Implement Azure Functions
- [Section-4-Develop-Azure-Compute-Solutions-Azure-Web-Apps-and-Azure-Functions](Section-4-Develop-Azure-Compute-Solutions-Azure-Web-Apps-and-Azure-Functions)

## Module 03: Develop solutions that use blob storage
- [Section-6-Develop-for-Azure-Storage](Section-6-Develop-for-Azure-Storage)

## Module 04: Develop solutions that use Cosmos DB storage
- [Section-6-Develop-for-Azure-Storage](Section-6-Develop-for-Azure-Storage)

## Module 05: Implement IaaS solutions
- [Section-3-Develop-Azure-Compute-Solutions-Virtual-Machines](Section-3-Develop-Azure-Compute-Solutions-Virtual-Machines)

- [Section-5-Develop-Azure-Compute-Solutions-Docker-Azure-Container-Instances-Kubernetes](Section-5-Develop-Azure-Compute-Solutions-Docker-Azure-Container-Instances-Kubernetes)

## Module 06: Implement user authentication and authorization
- [Section-7-Implement-Azure-Security](Section-7-Implement-Azure-Security)

## Module 07: Implement secure cloud solutions
- [Section-7-Implement-Azure-Security](Section-7-Implement-Azure-Security)

## Module 08: Implement API Management
- [Section-9-Connect-to-And-Consume-Azure-and-Third-Party-Services](Section-9-Connect-to-And-Consume-Azure-and-Third-Party-Services)

## Module 09: Develop event-based solutions
- [Section-9-Connect-to-And-Consume-Azure-and-Third-Party-Services](Section-9-Connect-to-And-Consume-Azure-and-Third-Party-Services)

## Module 10: Develop message-based solutions
- [Section-9-Connect-to-And-Consume-Azure-and-Third-Party-Services](Section-9-Connect-to-And-Consume-Azure-and-Third-Party-Services)


## Module 11: Instrument solutions to support monitoring and logging
- [Section-8-Monitor-Troubleshoot-and-Optimize-Solutions](Section-8-Monitor-Troubleshoot-and-Optimize-Solutions)

## Module 12: Integrate caching and content delivery within solutions
- [Section-8-Monitor-Troubleshoot-and-Optimize-Solutions](Section-8-Monitor-Troubleshoot-and-Optimize-Solutions)
